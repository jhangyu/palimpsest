"""
---
name: test_ai_provider_migrations
description: "AI provider schema migration tests: table declarations, idempotency, and owner backfill"
stage: stage1
type: pytest
target:
  layer: backend
  domain: ai-provider-migrations
spec_doc: null
test_file: tests/stage1/test_ai_provider_migrations.py
functions:
  - name: test_table_declarations_match_frozen_defaults_and_constraints
    line: 169
    purpose: "Verifies all columns, server defaults, constraints, and indexes match the frozen schema"
    fixtures: []
  - name: test_schema_sql_is_idempotent_and_release_b_is_separate
    line: 200
    purpose: "Ensures expansion SQL is idempotent (IF NOT EXISTS) and release-B SET NOT NULL is separate"
    fixtures: []
  - name: test_user_secret_bootstrap_is_idempotent
    line: 210
    purpose: "bootstrap_user_secret_key is idempotent: repeat call for same user_id is a no-op"
    fixtures: [tmp_path]
  - name: test_existing_user_key_backfill_is_empty_safe_and_rerunnable
    line: 226
    purpose: "backfill_existing_user_secret_keys handles empty DB, multi-user, and re-runs safely"
    fixtures: [tmp_path]
  - name: test_owner_backfill_uses_deterministic_active_admin
    line: 257
    purpose: "backfill_site_owners assigns oldest active admin as owner and reports correct counts"
    fixtures: []
  - name: test_owner_backfill_without_admin_is_explicit_and_non_destructive
    line: 268
    purpose: "backfill_site_owners with no active admin returns NO_ACTIVE_ADMIN and makes no DB changes"
    fixtures: []
  - name: test_release_b_precondition_rejects_null_or_orphan_owner
    line: 278
    purpose: "validate_owner_release_b_preconditions raises OwnerContractPreconditionError on null or orphan owners"
    fixtures: []
run:
  command: "PYTHONPATH=.:backend:tests/stage1 python -m pytest tests/stage1/test_ai_provider_migrations.py -v"
  env:
