/**
 * UI Bugfix Review — Playwright tests (Batch 1 + Batch 2)
 *
 * Verifies all 7 tasks from the ui-bugfix-team:
 *   T1: Footer copyright removed
 *   T2: Sidebar template nav sections removed
 *   T3: Sidebar footer data-session attrs + Upgrade/Billing removed
 *   T4: Topbar offcanvas Billing removed
 *   T5: Avatar rendering fix (session.ts + users.ts)
 *   T6: AI Tokens moved from settings → security page
 *   T7: 401/404/500 error pages deployed
 *
 * Error-page tests (T7) run without auth.
 * Admin-layout tests require a valid session — marked skip until
 * a session fixture is provided in Phase 8.
 *
 * Run: npx playwright test tests/ui-bugfix.spec.ts
 */
