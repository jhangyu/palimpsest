...
def decode_vue_gallery(html_content: str) -> str:
    ...
    for gallery in galleries:
        ...
        for item in collection:
            ...
            if img_url:
                new_html += f'<li><img src="{img_url}" alt="{title}" loading="lazy">'
                if title:
                    new_html += f'<span class="caption">{title}</span>'
                new_html += '</li>'

        new_html += '</ul>'

        if data.get('from'):
            new_html += f'<div class="credit">{data["from"]}</div>'
...
ALLOWED_UL_ATTRS = {'class'}  # ul only allows class for gallery
ALLOWED_LI_ATTRS = set()  # li 不允許屬性
ALLOWED_DIV_ATTRS = {'class'}  # div only allows class for credit container
